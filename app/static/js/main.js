...
function bindEventHandlers() {
    ...
    $('#calculate-formula-btn').click(function() {
        ...
        console.log("傳送資料 - 閒家牌:", playerCards, "莊家牌:", bankerCards);
        
        // 將連續輸入的數字轉換為數字陣列
        const playerCardsArray = playerCards.split('').map(Number);
        const bankerCardsArray = bankerCards.split('').map(Number);
        ...
        $.ajax({
            ...
            contentType: 'application/json',
            data: JSON.stringify({
                banker_cards: bankerCardsArray,
                player_cards: playerCardsArray
            }),
            success: function(data) {
                console.log("接收到的響應:", data);
                if (data.success) {
                    // 更新公式預測結果
                    let prediction = data.prediction;  // 修正欄位名
                    let confidence = data.confidence;  // 修正欄位名
                    ...
                    // 更新頻率和優勢值顯示
                    $('#banker-frequency').text(data.banker_frequency || 0);
                    $('#player-frequency').text(data.player_frequency || 0);
                    $('#tie-frequency').text(data.tie_frequency || 0);
...
$('#add-with-cards-btn').click(function() {
    ...
    // 自動分析牌型
    $('#player-cards, #banker-cards').on('input', function() {
        // 獲取閒家牌和莊家牌
        ...
        if (playerCards.length >= 2 && bankerCards.length >= 2) {
            // 驗證輸入是否有效
            let allValid = true;
            for (const card of [...playerCards, ...bankerCards]) {
                if (isNaN(card) || card < 0 || card > 9) {
                    allValid = false;
                    break;
                }
            ...
            if (allValid) {
                ...
                // 自動更新分析
                updateCardAnalysis(playerCards, bankerCards);
            }
        }
    });
    
    function updateCardAnalysis(playerCardsParam, bankerCardsParam) {
        // 如果提供了引數，使用引數；否則從DOM獲取
        let playerCards = playerCardsParam;
        let bankerCards = bankerCardsParam;
        ...
        if (!playerCards || !bankerCards) {
            ...
            // 如果輸入為空，不進行分析
            if (!playerCardsInput || !bankerCardsInput) {
                return;
            }
            
            // 將輸入字串轉換為數字陣列 - 修改為正確處理連續輸入
            playerCards = playerCardsInput.split('').map(Number);
            bankerCards = bankerCardsInput.split('').map(Number);
        }
        
        // 驗證輸入是否有效
        if (playerCards.some(isNaN) || bankerCards.some(isNaN)) {
            console.error('輸入包含非數字字元');
            return;
        }
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data.success) {
                    // 更新頻率顯示
                    $('#banker-frequency').text(data.banker_frequency);
                    $('#player-frequency').text(data.player_frequency);
                    $('#tie-frequency').text(data.tie_frequency || 0);
                    
                    // 更新優劣勢牌值
                    $('#banker-advantage').text(data.banker_advantage);
                    $('#player-advantage').text(data.player_advantage);
                    ...
                    // 更新赫茲結果
                    $('#hertz-result').text(data.hertz_result || prediction || '無預測');
                } else {
                    console.error('計算錯誤:', data.message);
                    // 清空結果
                    $('#banker-frequency, #player-frequency, #tie-frequency, #banker-advantage, #player-advantage').text('0');
                    $('#formula-prediction').html('<div class="alert alert-secondary">無法計算</div>');
                    $('#hertz-result').text('無預測');
                }
            },
            error: function(xhr, status, error) {
                console.error("API請求失敗:", status, error);
                // 清空結果
                $('#banker-frequency, #player-frequency, #tie-frequency, #banker-advantage, #player-advantage').text('0');
                $('#formula-prediction').html('<div class="alert alert-secondary">請求失敗</div>');
                $('#hertz-result').text('無預測');
            }
        });
    }
