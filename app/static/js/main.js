...
function bindEventHandlers() {
    ...
    $('#predict-btn').click(function() {
        $.ajax({
            url: '/api/predict/ai',  // 使用AI预测专用API
            type: 'POST',
            contentType: 'application/json',
            data: JSON.stringify({
            ...
            success: function(data) {
                if (data.success) {
                    ...
                    $('#ai-prediction').text('預測失敗: ' + data.message);
                }
            },
            error: function() {
                $('#ai-prediction').text('預測請求失敗');
            }
        });
    });
    ...
    $('#formula-predict-btn').click(function() {
        $.ajax({
            url: '/api/predict/formula',  // 使用算牌公式预测专用API
            type: 'POST',
            contentType: 'application/json',
            data: JSON.stringify({
                history: gameHistory.map(record => record.result)
    ...
    $('#calculate-formula-btn').click(function() {
        ...
        // 解析輸入
        const bankerCards = bankerCardsInput ? bankerCardsInput.split(/\s+/).map(Number) : [];
        const playerCards = playerCardsInput ? playerCardsInput.split(/\s+/).map(Number) : [];
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.success) {
                    ...
                    $('#tie-frequency').text(data.tie_frequency);
                    ...
                    // 更新赫茲結果
                    let hertzResult = data.hertz_result;
...
$('#add-with-cards-btn').click(function() {
    ...
    function calculateBaccaratPoints(cards) {
        ...
        // A=1, 2-9=對應點數, 10/J/Q/K=0
        let sum = 0;
       
        for (const card of cards) {
            if (card === 1) { // A
                sum += 1;
            } else if (card >= 2 && card <= 9) { // 2-9
                sum += card;
            } else { // 10, J, Q, K
                sum += 0;
            }
